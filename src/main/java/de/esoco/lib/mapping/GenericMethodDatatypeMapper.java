...
/********************************************************************
 * <p>Generic implementation of the MethodDatatypeMapper interface that is based
 * on a mapping table of datatype classes. It is abstract because the mapping of
 * values must be implemented by subclasses. The mapping methods will be invoked
 * through reflection. So, to provide a certain mapping a subclass needs to do
 * the following:</p>
 *
 * <ol>
	...
	implements MethodDatatypeMapper
{
	...
	private Map<Class<?>, MappedTypes> aMapping =
	 ...
	 * would be</p>
	 *
	 * <pre>public x.y.NewType mapOldType(Object rTarget, Method rMethod, a.b.OldType)</pre>
	 *
	 ...
	 * implementations.</p>
	 *
	 * <p>The method may also be static, e.g. if the application needs to invoke
	 * the mapping methods directly and doesn't want to create a separate
	 * instance.</p>
	 *
	 * @throws IllegalArgumentException If the target method for the dispatching
	 *                                  could not be found
	 *
	 * @see    de.esoco.lib.mapping.MethodDatatypeMapper#mapValue(Object,
	 *         Method, Object)
	 */
	@Override
	public Object mapValue(Object rTarget, Method rMethod, Object rValue)
	{
		...
		return ReflectUtil.invokePublic(this,
										sMethod,
										new Object[]
										{
											rTarget, rMethod, rValue
										},
										new Class[]
										{
	...
	/***************************************
	 * Internal method that initializes the mapping for a certain datatype class
	 * that has not been defined explicitly. If a mapping exist for either a
	 * superclass, an interface, or a super-interface of the datatype class that
